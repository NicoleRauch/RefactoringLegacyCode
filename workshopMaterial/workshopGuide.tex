...
\texttt{BalanceAndAverage} is created

\texttt{PushingBalancesCalculator.fillData()}


\subsection*{Step 02: Preparing for method extraction: Resolving ambiguous return values}

\subsubsection*{Observations:}
\begin{itemize}
\item If we tried to extract the body of the outer loop into a method, we would be told that this method manipulates two variables which cannot both be returned, namely \texttt{balance} and \texttt{latestBalance}.
\end{itemize}
...
If we write \texttt{balance} to \texttt{balanceAndAverage} at the end of the loop, we can read \texttt{balance} from \texttt{balanceAndAverage} at the beginning of the loop.
\item Regarding \texttt{lastBalance}: \\
At the end of the loop, \texttt{lastBalance} equals \texttt{balance}, so we can set \texttt{lastBalance} to \texttt{balance} at the beginning of the loop.
\end{itemize}
...
\texttt{PushingBalancesCalculator.calculateValuesForMonth()} is moved to\\ \texttt{BalanceAndAverage.calculateValues()}

\texttt{PushingBalancesCalculator.calculateProportionalBalance()} is moved to\\ \texttt{BalanceAndAverage.calculateProportionalBalance()}
...
\subsubsection*{Observations:}
\begin{itemize}
\item The for loop reuses the same \texttt{BalanceAndAverage} object in each loop iteration.
...
\texttt{BalanceAndAverage.calculateValues()}
...
\texttt{BalanceAndAverage} (fields, constructor, method signature)
...
\texttt{BalanceAndAverage.calculateBalance()} is created

\texttt{BalanceAndAverage.calculateAverageBalance()} is created 

\texttt{BalanceAndAverage.setBalanceAndAverage()} is removed

\texttt{BalanceAndAverage.calculateValues()} is modified

\subsection*{Step 09: Return Values from Calculation Methods}
\subsubsection*{Observations:}
\begin{itemize}
\item The two calculation methods do calculation and setting of the fields. 
\end{itemize}
...
\texttt{BalanceAndAverage.calculateBalance()} 

\texttt{BalanceAndAverage.calculateAverageBalance()} 
...
\texttt{BalanceAndAverage} (fields and \texttt{calculate...} methods are removed)
...
\texttt{BalanceAndAverage.getAverageBalance()}

\texttt{BalanceAndAverage.calculateProportionalBalance()} is removed

\texttt{BalanceAndAverage.rateOf()} is created
...
\texttt{BalanceAndAverage} is converted to \texttt{Month} (with variants)
